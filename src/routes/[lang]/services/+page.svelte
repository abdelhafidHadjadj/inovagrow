<script>
  ...
  const services = [
    ...
    {
      title: t(lang, 'services.graphic.title'),
      description: t(lang, 'services.graphic.short_descript'),
      image: techConsultingImg,
      icon: iconBrain,
      link: `/${lang}/services/graphic-design`
    ...
    {
      title: t(lang, 'services.devops.title'),
      description: t(lang, 'services.devops.short_descript'),
      image: techConsultingImg,
      icon: iconBrain,
...
<section class="relative bg-[var(--color-light)] bg-grid mb-6 mt-14">
  <!-- Background SVG and overlay -->
   <div class="relative bg-[var(--color-light)] flex flex-col py-16 text-black">
       ...
       <div class="flex flex-col justify-center items-center gap-6 z-1 text-center">
        <h1 class="text-3xl md:text-5xl font-bold mb-4">{t(lang, 'services.title')}</h1>
        <p class="text-sm md:text-base text-gray-700 leading-relaxed max-w-prose">{t(lang, 'services.description')}</p>
        </div>
    </div>

  <!-- Content wrapper -->
  <div class="relative z-10 max-w-7xl mx-auto flex flex-col items-start gap-12">
